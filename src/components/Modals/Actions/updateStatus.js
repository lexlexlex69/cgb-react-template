...
const UpdateStatusModal = ({ modal, toggle, modalData, history, setModalData, params }) => {
  const dispatch = useDispatch()
  const statusList = useSelector((state) => state.status.requestStatusList)
  const [statusOptions, setStatusOptions] = useState()
  useEffect(() => {
    if (statusList.length !== 0 && modalData) {
      const options = statusList.filter((item) => item.value !== modalData.status_id)
    ...
    onSubmit: (values) => {
      Swal.fire({
        ...
        if (result.value) {
          ...
          axios.post('api/admin/update-request-status', {
