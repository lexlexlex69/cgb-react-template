...
import { Modal, ModalHeader, ModalBody, ModalFooter, Button, Label, Form, Row, Input, Col, FormFeedback } from 'reactstrap';
...
const CancelRequestModal = ({ modal, toggle, modalData, history, setModalData, params }) => {
  ...
  const validation = useFormik({
    ...
    validationSchema: Yup.object({
      reason: Yup.string().max(250, 'Maximum of 250 characters').required("Please Enter reason of cancellation"),
    }),
    onSubmit: (values) => {
      Swal.fire({
        ...
        if (result.value) {
          ...
          axios.post('api/admin/cancel-request', {
