...
const ForwardRequestModal = ({ modal, toggle, modalData, history, setModalData, params }) => {
  ...
  useEffect(() => {
    if (divisionList.length !== 0 && modalData) {
      const options = divisionList.filter((item) => item.divisions_id !== modalData.assigned_division)
    ...
    onSubmit: (values) => {
      Swal.fire({
        ...
        if (result.value) {
          ...
          axios.post('api/admin/forward-request', {
