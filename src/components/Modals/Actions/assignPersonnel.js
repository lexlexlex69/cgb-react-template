...
import { Modal, ModalHeader, ModalBody, ModalFooter, Button, Form, Row, Col } from 'reactstrap';
...
const AssignPersonnelModal = ({ modal, toggle, modalData, history, setModalData, params }) => {
  ...
  useEffect(() => {
    if (personnelList.length !== 0 && modalData) {
      let options = personnelList.filter((item) => item.division_id === modalData.assigned_division)
      ...
      const personnelOptions = options.map((item) => {
        var name = item.first_name + ' ' + (item.middle_name ? (item.middle_name.charAt(0) + '. ') : '') + item.last_name + (item.suffix ? ', ' + item.suffix : '')
  ...
  const validation = useFormik({
    ...
    validationSchema: Yup.object({
      selectedPersonnel: Yup.array().min(1, 'Minimum of 1 Personnel').required('Please select a personnel'),
      actionTaken: Yup.string().max(250, 'Maximum of 250 characters').required('Please enter action/reason'),
    }),
    onSubmit: (values) => {
      // return;
      Swal.fire({
        ...
        if (result.value) {
          ...
          axios.post('api/admin/assign-personnel', {
    ...
    <div>
      {
        modalData && (
          ...
          >
            ...
            <ModalHeader toggle={() => { toggle(); validation.resetForm() }} tag="h4">
                    ...
                    <Row>
                      <Select2InputField
                        ...
                        onChangeFunctions={(personnelOptions) => {
                          validation.setFieldValue("selectedPersonnel", personnelOptions)
                        }}
                      />
                    </Row>
                    <Row>
            ...
            <ModalFooter>
              <Button color="secondary" onClick={() => { toggle(); validation.resetForm() }}>
                Cancel
              </Button>
            </ModalFooter>
          </Modal>
