...
import { Button, Modal, ModalHeader, ModalBody, ModalFooter, Table, Badge, Tooltip, Placeholder } from 'reactstrap';
import axios from 'axios';
import Swal from 'sweetalert2';
import moment from 'moment';
import { Buffer } from 'buffer';
const UserDetails = ({ modal, toggle, modalData, setModalData, history, params }) => {
    //Tooltip
    ...
    const toggleToolTipDisapprove = () => setTooltipOpenDisapprove(!tooltipOpenDisapprove);
    //***** */
    ...
    const userStatusUpdate = (status, id) => {
        Swal.fire({
            ...
            if (result.value) {
                ...
                axios.post('api/admin/update-user-status', {
                    'status': status,
                    'id': id,
                }).then(res => {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'User has been' + (status === 'Approve' ? 'Approved' : 'Disapproved'),
                        showConfirmButton: false,
                        timer: 1500
                    }).then(function () {
    ...
    useEffect(() => {
        if (modalData&&modalData.uploaded_files.length !== 0) {
            setImgLoading(true);
            axios.get('api/admin/get-uploaded-id/' + modalData.uploaded_files[0].uploaded_files_id, {
        ...
        <div>
            ...
            >
                ...
                <ModalBody>
                    ...
                    <div>
                        {
                            modalData && ((modalData.status_id === 3 || modalData.status_id === 2) && (
                                <>
                                    <button
                                        type="button"
                                        className="btn btn-success"
                                        id="TooltipForApprove"
                                        onClick={() => { userStatusUpdate('Approve', modalData.id) }}
                                    >
                                    ...
                                    {" "}
                                    {
                                        (!modalData.status_id === 2 || modalData.status_id === 3) && (
                                            <>
                                                <button
                                                    type="button"
                                                    className="btn btn-danger"
                                                    id="TooltipForDisapprove"
                                                    onClick={() => { userStatusUpdate('Disapprove', modalData.id) }}
                                                >
                ...
                <ModalFooter>
                    <Button color="secondary" onClick={() => { toggle(), setImgSrc() }}>
                        Close
                    </Button>
                </ModalFooter>
            </Modal>
