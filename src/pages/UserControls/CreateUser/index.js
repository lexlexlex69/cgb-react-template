...
import { Card, CardBody, CardHeader, Container, Form, Row, Col, Alert } from "reactstrap";
//Import Breadcrumb
  ...
  useEffect(() => {
    ...
    if (dropDownOptions.userRoles) {
      if (userDetails.role === 'Administrator') {
        const options = dropDownOptions.userRoles.filter((item) => item.user_roles_id !== 2)
        ...
        const rolesList = dropDownOptions.userRoles.filter((item) => item.user_roles_id !== 1 && item.user_roles_id !== 3)
  ...
  const validation = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      division: Yup.object().when('userRole', {
        is: (userRole) => userRole.value !== 1 && userDetails.role !== 'Division Head',
    ...
    onSubmit: (values) => {
      Swal.fire({
        ...
        if (result.value) {
          ...
          axios.post('api/admin/create-user', values, {
            credentials: 'include'
          }).then(res => {
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid className="vh-100 d-flex flex-column">
          <Breadcrumbs
            title="Create User"
            breadcrumbItems={[{ title: "User Management" }, { title: "User Controls" }, { title: "Create User" }]}
          />
          <Card>
            <CardHeader>
            ...
            <CardBody>
              <div className="p-2">
                ...
                >
                  ...
                  {
                    userDetails && (
                      userDetails.role === 'Administrator' && validation.values.userRole.value !== 1 ? (
                        <Row>
                          <Select2InputField
                            col={"12"}
                            label={"Division"}
                    ...
                    <Col md="12">
                      {
                        failErrorMessage && (
                          <Alert
                            ...
                            role="alert"
                          >
                            <i className="mdi mdi-account-cancel me-2"></i>{failErrorMessage}
                          </Alert >
                        )
                      }
                    </Col>
